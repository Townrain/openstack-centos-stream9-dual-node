...
remote_setup_compute() {
    ...
    ssh -o StrictHostKeyChecking=no \
        "${COMPUTE_USER}@${COMPUTE_IP}" \
        "bash /root/openstack_base_env.sh \
            --remote \
            '${COMPUTE_HOSTNAME}' \
            '${COMPUTE_MGMT_IFACE}' \
            '${COMPUTE_MGMT_IP}' \
...
interactive_main() {
    ...
    echo ""
    [ "$MYSQL_ROOT_PASS" != "$MYSQL_ROOT_PASS_CONFIRM" ] && { log_error "MySQL 密码不一致"; exit 1; }

    read -r -s -p "OpenStack admin 密码 (Keystone/Dashboard 登录): " ADMIN_PASS
    echo ""
    read -r -s -p "确认 admin 密码: " ADMIN_PASS_CONFIRM
    echo ""
    [ "$ADMIN_PASS" != "$ADMIN_PASS_CONFIRM" ] && { log_error "admin 密码不一致"; exit 1; }

    # ---------- 服务密码 ----------
    echo ""
    echo "===== 服务密码 ====="
    echo "  为简化部署，建议所有 OpenStack 服务使用同一密码"
    read -r -p "  是否所有服务使用统一密码? [Y/n]: " USE_UNIFIED
    USE_UNIFIED="${USE_UNIFIED:-Y}"

    if [[ "$USE_UNIFIED" =~ ^[Yy]$ ]]; then
        read -r -s -p "  统一服务密码 [默认=${ADMIN_PASS}]: " SERVICE_PASS
        echo ""
        SERVICE_PASS="${SERVICE_PASS:-${ADMIN_PASS}}"
        RABBIT_PASS="${SERVICE_PASS}"
        KEYSTONE_DBPASS="${SERVICE_PASS}"
        GLANCE_DBPASS="${SERVICE_PASS}"
        GLANCE_PASS="${SERVICE_PASS}"
        PLACEMENT_DBPASS="${SERVICE_PASS}"
        PLACEMENT_PASS="${SERVICE_PASS}"
        NOVA_PASS="${SERVICE_PASS}"
        NEUTRON_PASS="${SERVICE_PASS}"
        CINDER_PASS="${SERVICE_PASS}"
        SWIFT_PASS="${SERVICE_PASS}"
        log_info "所有服务将使用统一密码"
    else
        read -r -s -p "  统一服务密码 (RPC/数据库等) [${ADMIN_PASS}]: " SERVICE_PASS
        echo ""
        SERVICE_PASS="${SERVICE_PASS:-${ADMIN_PASS}}"
        RABBIT_PASS="${SERVICE_PASS}"

        echo ""
        echo "  --- 各服务密码 (回车使用上面统一密码) ---"
        read -r -s -p "  Keystone DB  [${SERVICE_PASS}]: " KEYSTONE_DBPASS; echo ""
        KEYSTONE_DBPASS="${KEYSTONE_DBPASS:-${SERVICE_PASS}}"
        read -r -s -p "  Glance       [${SERVICE_PASS}]: " input; GLANCE_PASS="${input:-${SERVICE_PASS}}"; echo ""
        GLANCE_DBPASS="${GLANCE_PASS}"
        read -r -s -p "  Placement    [${SERVICE_PASS}]: " input; PLACEMENT_PASS="${input:-${SERVICE_PASS}}"; echo ""
        PLACEMENT_DBPASS="${PLACEMENT_PASS}"
        read -r -s -p "  Nova         [${SERVICE_PASS}]: " NOVA_PASS; echo ""
        NOVA_PASS="${NOVA_PASS:-${SERVICE_PASS}}"
        read -r -s -p "  Neutron      [${SERVICE_PASS}]: " NEUTRON_PASS; echo ""
        NEUTRON_PASS="${NEUTRON_PASS:-${SERVICE_PASS}}"
        read -r -s -p "  Cinder       [${SERVICE_PASS}]: " CINDER_PASS; echo ""
        CINDER_PASS="${CINDER_PASS:-${SERVICE_PASS}}"
        read -r -s -p "  Swift        [${SERVICE_PASS}]: " SWIFT_PASS; echo ""
