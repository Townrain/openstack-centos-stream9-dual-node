...
# ==================== 远程模式：计算节点配置 ====================
remote_compute_mode() {
    local ctrl_hostname="${1:-${CTRL_HOSTNAME:-}}"
    local ctrl_mgmt_ip="${2:-${CONTROLLER_IP:-}}"
    local compute_mgmt_ip="${3:-${COMPUTE_IP:-}}"
    local int_iface="${4:-${INT_IFACE:-ens34}}"
    local neutron_pass="${5:-${NEUTRON_PASS:-}}"
    local rabbit_pass="${6:-${RABBIT_PASS:-}}"
    local metadata_secret="${7:-${METADATA_SECRET:-}}"
    ...
    dnf install -y openstack-neutron-openvswitch || dnf install -y --allowerasing openstack-neutron-openvswitch || { log_error "安装 openstack-neutron-openvswitch 失败"; exit 1; }
...
NMEOF
    ...
    if [ -f "$nova_conf" ]; then
        ...
        safe_ini_write "$nova_conf" "neutron" "auth_url" "http://${ctrl_hostname}:5000/v3"
        safe_ini_write "$nova_conf" "neutron" "auth_type" "password"
        safe_ini_write "$nova_conf" "neutron" "project_domain_name" "Default"
        safe_ini_write "$nova_conf" "neutron" "user_domain_name" "Default"
        safe_ini_write "$nova_conf" "neutron" "region_name" "RegionOne"
        safe_ini_write "$nova_conf" "neutron" "project_name" "service"
        safe_ini_write "$nova_conf" "neutron" "username" "neutron"
        safe_ini_write "$nova_conf" "neutron" "password" "${neutron_pass}"
...
remote_setup_compute() {
    ...
    remote_int_iface=$(ssh -o BatchMode=yes -o ConnectTimeout=10 "${COMPUTE_USER}@${COMPUTE_IP}" \
        "for i in \$(ip -o link show up 2>/dev/null | awk -F': ' '{print \$2}' | tr -d '@' | grep -vE '^(lo|virbr|docker|br-|veth|tun|tap|vnet|ovs-)'); do [ \"\$i\" != '${remote_def_iface}' ] && echo \$i && break; done" 2>/dev/null || echo "ens34")
    ...
    ssh -o StrictHostKeyChecking=no "${COMPUTE_USER}@${COMPUTE_IP}" \
        "export NEUTRON_PASS='${NEUTRON_PASS}' RABBIT_PASS='${RABBIT_PASS}' \
         METADATA_SECRET='${METADATA_SECRET}' CTRL_HOSTNAME='${CTRL_HOSTNAME}' \
         CONTROLLER_IP='${CONTROLLER_IP}' COMPUTE_IP='${COMPUTE_IP}' \
         INT_IFACE='${remote_int_iface}'; \
         bash /root/openstack_neutron.sh --remote"
...
install_packages() {
    ...
    dnf install -y openstack-neutron openstack-neutron-ml2 openstack-neutron-openvswitch ebtables || \
        dnf install -y --allowerasing openstack-neutron openstack-neutron-ml2 openstack-neutron-openvswitch ebtables || \
        { log_error "安装 Neutron 软件包失败"; exit 1; }
...
configure_nova_neutron() {
    ...
    backup_file "$conf"
    safe_ini_write "$conf" "neutron" "auth_url" "http://${CTRL_HOSTNAME}:5000/v3"
    safe_ini_write "$conf" "neutron" "auth_type" "password"
    safe_ini_write "$conf" "neutron" "project_domain_name" "Default"
    safe_ini_write "$conf" "neutron" "user_domain_name" "Default"
    safe_ini_write "$conf" "neutron" "region_name" "RegionOne"
    safe_ini_write "$conf" "neutron" "project_name" "service"
    safe_ini_write "$conf" "neutron" "username" "neutron"
    safe_ini_write "$conf" "neutron" "password" "${NEUTRON_PASS}"
    safe_ini_write "$conf" "neutron" "service_metadata_proxy" "true"
    safe_ini_write "$conf" "neutron" "metadata_proxy_shared_secret" "${METADATA_SECRET}"
    log_info "nova.conf [neutron] 已追加"
}
...
interactive_main() {
    ...
    if [ "$NON_INTERACTIVE" -eq 1 ]; then
        ...
        if [ -z "${METADATA_SECRET:-}" ]; then
            log_error "METADATA_SECRET 未设置。请先运行基础环境模块或手动设置 /root/openstack_env.conf 中的 METADATA_SECRET"
            exit 1
        fi
    else
        echo "========== 控制节点 =========="
