...
safe_ini_write() {
    ...
    if command -v crudini &>/dev/null; then
        ...
        # Fallback: sed-based upsert (section-scoped)
        if grep -q "^\[${section}\]" "$file" 2>/dev/null; then
            if sed -n "/^\[${section}\]/,/^\[/p" "$file" | grep -q "^${key}[[:space:]]*="; then
                # Key exists → update in-place within section
                sed -i "/^\[${section}\]/,/^\[/ s|^${key}[[:space:]]*=.*|${key} = ${value}|" "$file"
            else
                # Section exists but key missing → append after section header
...
detect_int_ip() {
    local def_iface
    def_iface=$(ip -4 route show default 2>/dev/null | awk '{print $5}' | head -1)
    for iface in $(ip -o link show up 2>/dev/null | awk -F': ' '{print $2}' | tr -d '@' | grep -vE '^(lo|virbr|docker|br-|veth|tun|tap|vnet|ovs-)'); do
...
register_cleanup() {
    [ "$_cleanup_registered" -eq 1 ] && return
...
cleanup() {
    local exit_code=$?
    # Restore network repos if in offline mode
    type restore_network_repos &>/dev/null && restore_network_repos 2>/dev/null || true
    # Clean temp files
    rm -f /tmp/openstack_*.tmp 2>/dev/null || true
    log_info "清理完成 (exit=$exit_code)"
    return $exit_code
}
