...
get_remote_ifaces() {
    ssh -o BatchMode=yes -o ConnectTimeout=10 \
        ...
        "ip -o link show up 2>/dev/null | awk -F': ' '{print \$2}' | tr -d '@' | grep -vE '^(lo|virbr|docker|br-|veth|tun|tap|vnet|ovs-)'" 2>/dev/null
}
...
NMEOF
    ...
    # 激活连接
    if nmcli connection up ifname "${iface}" 2>/dev/null; then
        : # 激活成功
    else
        # 连接可能未被 NM 识别，尝试按名称激活
        nmcli connection up "${iface}" 2>/dev/null || {
            log_warn "首次激活 ${iface} 失败，等待 NM 重新加载..."
            sleep 2
            nmcli connection reload
            nmcli connection up ifname "${iface}" 2>/dev/null || true
        }
    fi
...
setup_repos_and_packages() {
    ...
    if [ -n "${OFFLINE_REPO_PATH:-}" ]; then
        ...
        dnf config-manager --set-enabled openstack-offline 2>/dev/null || true
...
# ========== 网络 ==========
CTRL_MGMT_IFACE="${CTRL_MGMT_IFACE:-}"
CTRL_MGMT_IP="${CTRL_MGMT_IP:-}"
CTRL_MGMT_PREFIX="${CTRL_MGMT_PREFIX:-24}"
CTRL_MGMT_GW="${CTRL_MGMT_GW:-}"
CTRL_INT_IFACE="${CTRL_INT_IFACE:-}"
CTRL_INT_IP="${CTRL_INT_IP:-}"
CTRL_INT_PREFIX="${CTRL_INT_PREFIX:-24}"
CTRL_INT_GW="${CTRL_INT_GW:-}"
CTRL_EXT_IP="${CTRL_EXT_IP:-}"
INT_IP="${CTRL_INT_IP:-}"
...
COMPUTE_INT_GW="${COMPUTE_INT_GW:-}"
COMPUTE_MGMT_GW="${COMPUTE_MGMT_GW:-}"
COMPUTE_INT_PREFIX="${COMPUTE_INT_PREFIX:-24}"
COMPUTE_MGMT_PREFIX="${COMPUTE_MGMT_PREFIX:-24}"
...
auto_detect_local_network() {
    # 已从 env.conf 加载 → 直接复用
    if [ -n "${CTRL_MGMT_IFACE:-}" ] && [ -n "${CTRL_INT_IFACE:-}" ]; then
        echo ""
        log_info "复用已有网络配置:"
        echo "  管理: ${CTRL_MGMT_IFACE}  ${CTRL_MGMT_IP}/${CTRL_MGMT_PREFIX}  gw=${CTRL_MGMT_GW:-无}"
        echo "  内部: ${CTRL_INT_IFACE}  ${CTRL_INT_IP}/${CTRL_INT_PREFIX}  gw=${CTRL_INT_GW:-无}"
...
auto_detect_remote_network() {
    # 已从 env.conf 加载 → 直接复用
    if [ -n "${COMPUTE_MGMT_IFACE:-}" ] && [ -n "${COMPUTE_INT_IFACE:-}" ]; then
        echo ""
        log_info "复用计算节点已有网络配置:"
        echo "  管理: ${COMPUTE_MGMT_IFACE}  ${COMPUTE_MGMT_IP}/${COMPUTE_MGMT_PREFIX:-24}  gw=${COMPUTE_MGMT_GW:-无}"
        echo "  内部: ${COMPUTE_INT_IFACE}  ${COMPUTE_INT_IP}/${COMPUTE_INT_PREFIX:-24}  gw=${COMPUTE_INT_GW:-无}"
        return 0
    fi
...
interactive_main() {
    ...
    # env.conf 已存在 = 用过 = 一次确认就跳过所有提示
    if [ -f /root/openstack_env.conf ]; then
        log_info "检测到已有环境配置:"
        echo ""
        echo "  控制节点: ${CTRL_HOSTNAME:-控制器}  IP: ${CONTROLLER_IP:-未配置}"
        echo "  计算节点: ${COMPUTE_HOSTNAME:-计算节点}  IP: ${COMPUTE_IP:-未配置}"
        echo "  管理密码: ${ADMIN_PASS:0:2}**** (已隐藏)"
        echo ""
        if confirm "使用以上已有配置? (选 n 则重新填写)"; then
            log_info "已加载已有配置，跳过基础环境交互"
    ...
    if ! $skip_interactive; then
        echo ""
        echo "========== 主机名配置 =========="
        read -r -p "控制节点主机名 [${CTRL_HOSTNAME:-controller-63}]: " input
        CTRL_HOSTNAME="${input:-${CTRL_HOSTNAME:-controller-63}}"
        read -r -p "计算节点主机名 [${COMPUTE_HOSTNAME:-compute-63}]: " input
        COMPUTE_HOSTNAME="${input:-${COMPUTE_HOSTNAME:-compute-63}}"
    fi
    ...
    [ -z "${COMPUTE_IP:-}" ] && read -r -p "计算节点管理IP: " COMPUTE_IP
    [ -z "${COMPUTE_USER:-}" ] && read -r -p "SSH 用户名 [root]: " COMPUTE_USER
    COMPUTE_USER="${COMPUTE_USER:-root}"
    setup_ssh_keys
    auto_detect_remote_network
    ...
    fi  # end of if ! skip_interactive

    # 立刻落盘，不等后续部署（防止中途卡死丢失配置）
    save_env_config
    log_info "配置已保存至 /root/openstack_env.conf"
