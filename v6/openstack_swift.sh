...
remote_storage_mode() {
    ...
    dnf install -y openstack-swift-account openstack-swift-container openstack-swift-object \
        ...
        dnf install -y --allowerasing openstack-swift-account openstack-swift-container openstack-swift-object \
        xfsprogs rsync rsync-daemon || \
        { log_error "安装 Swift 存储组件失败"; exit 1; }
...
install_packages() {
    ...
    dnf install -y openstack-swift-proxy python3-swiftclient python3-keystoneclient || \
        dnf install -y --allowerasing openstack-swift-proxy python3-swiftclient python3-keystoneclient || \
        { log_error "安装 Swift Proxy 失败"; exit 1; }
...
[pipeline:main]
pipeline = catch_errors gatekeeper healthcheck proxy-logging cache container_sync bulk ratelimit authtoken keystoneauth tempurl formpost container-quotas account-quotas slo dlo versioned_writes symlink proxy-logging proxy-server
...
SWIFTCONF
    ...
    if $need_rebuild; then
        # Backup existing rings before rebuild
        if ls /etc/swift/*.ring.gz &>/dev/null 2>&1; then
            local _ring_bak="/etc/swift/ring-backup.$(date '+%Y%m%d%H%M%S')"
            mkdir -p "$_ring_bak"
            cp /etc/swift/*.ring.gz "$_ring_bak/" 2>/dev/null || true
            log_info "已备份 Ring 文件到 $_ring_bak"
        fi
        ...
        log_info "验证 Ring 配置..."
        for builder in account container object; do
            if [ ! -s "${builder}.builder" ]; then
                log_error "${builder}.builder 缺失或为空，中止 rebalance。备份在 /etc/swift/ring-backup.*"
                return 1
            fi
        done
