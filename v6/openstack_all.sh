#!/bin/bash
###############################################################################
# OpenStack Dalmatian 一键部署总脚本  v6
# 运行位置: 控制节点
# 运行用户: root
#
# 本地运行:
...
#
# 远程一键运行:
#   bash <(curl -sSL https://raw.githubusercontent.com/.../v6/openstack_all.sh) --keep
#   bash <(curl -sSL https://raw.githubusercontent.com/.../v6/openstack_all.sh)
###############################################################################
...
if [ -n "${_CANDIDATE_DIR:-}" ] && [ -f "${_CANDIDATE_DIR}/openstack_common.sh" ]; then
    ...
    SCRIPTS=(
        ...
        openstack_cleanup.sh          openstack_cleanup_verify.sh
    )
...
show_menu() {
    echo ""
    echo "╔══════════════════════════════════════════════════════════════╗"
    echo "║     OpenStack Dalmatian 一键部署 (CentOS Stream 9)  v6     ║"
    echo "╠══════════════════════════════════════════════════════════════╣"
    echo "║  模块:                                                      ║"
    for m in "${MODULES[@]}"; do
...
main() {
    ...
    case "$CHOICE" in
        ...
        [Vv])
            ...
            if [ -f "${SCRIPT_DIR}/openstack_cleanup_verify.sh" ]; then
                echo -e "\n${BLUE}--- 清理验证 ---${NC}"
                bash "${SCRIPT_DIR}/openstack_cleanup_verify.sh" --non-interactive || true
            fi
            ;;

        [Cc])
            load_env
            if [ -f "${SCRIPT_DIR}/openstack_cleanup.sh" ]; then
                bash "${SCRIPT_DIR}/openstack_cleanup.sh"
            else
                log_error "清理脚本不存在"
            fi
            ;;
