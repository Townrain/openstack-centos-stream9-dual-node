...
remote_setup_compute() {
    ...
    remote_ifaces=$(ssh -o BatchMode=yes -o ConnectTimeout=10 "${COMPUTE_USER}@${COMPUTE_IP}" \
        "ip -o link show up 2>/dev/null | awk -F': ' '{print \$2}' | tr -d '@' | grep -vE '^(lo|virbr|docker|br-|veth|tun|tap|vnet|ovs-)'" 2>/dev/null || echo "")
    ...
    ssh -o StrictHostKeyChecking=no "${COMPUTE_USER}@${COMPUTE_IP}" \
        "export NOVA_PASS='${NOVA_PASS}' RABBIT_PASS='${RABBIT_PASS}' \
         PLACEMENT_PASS='${PLACEMENT_PASS}' NEUTRON_PASS='${NEUTRON_PASS}' \
         CTRL_HOSTNAME='${CTRL_HOSTNAME}' COMPUTE_IP='${COMPUTE_IP}' \
         CONTROLLER_IP='${CONTROLLER_IP}'; \
         bash /root/openstack_nova.sh --remote"
...
remote_compute_mode() {
    ...
    local compute_mgmt_ip="${2:-${COMPUTE_IP:-}}"
    local nova_pass="${3:-${NOVA_PASS:-}}"
    local rabbit_pass="${4:-${RABBIT_PASS:-}}"
    local placement_pass="${5:-${PLACEMENT_PASS:-}}"
    local neutron_pass="${6:-${NEUTRON_PASS:-123456}}"
    local ctrl_mgmt_ip="${7:-${CONTROLLER_IP:-192.168.63.10}}"
...
NOVAEOF2
    ...
    local _existing_id
    _existing_id=$(cat /var/lib/nova/compute_id 2>/dev/null | tr -d '[:space:]')
    if [[ "$_existing_id" =~ ^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$ ]]; then
        log_info "compute_id 已存在且有效: $_existing_id"
    else
        uuidgen > /var/lib/nova/compute_id
        log_info "compute_id 已生成: $(cat /var/lib/nova/compute_id)"
    fi
...
sync_database() {
    ...
    if su -s /bin/sh -c "nova-manage cell_v2 list_cells" nova 2>/dev/null | grep -qw cell0; then
        log_info "cell0 已注册，跳过"
    else
        log_info "注册 cell0..."
        su -s /bin/sh -c "nova-manage cell_v2 map_cell0" nova
    fi

    if su -s /bin/sh -c "nova-manage cell_v2 list_cells" nova 2>/dev/null | grep -qw cell1; then
        log_info "cell1 已存在，跳过"
    else
        log_info "创建 cell1..."
