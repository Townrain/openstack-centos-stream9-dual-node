...
check "openstack-glance-api 已安装"       "systemctl list-unit-files 2>/dev/null | grep -qE 'openstack-glance-api\.(service|timer)'"
check_warn "openstack-glance-api 运行中"   "systemctl is-active openstack-glance-api"

if ! systemctl is-active openstack-glance-api &>/dev/null; then
    echo -e "  Glance 服务未运行      ${WARN}"
    WARN_COUNT=$((WARN_COUNT + 1))
    if journalctl -u openstack-glance-api --no-pager -n 5 2>/dev/null | grep -q .; then
        echo "  --- 最近日志 ---"
        journalctl -u openstack-glance-api --no-pager -n 5 2>/dev/null || true
    fi
else
    echo -e "  Glance 运行状态                          ${PASS}"
    PASS_COUNT=$((PASS_COUNT + 1))
fi
...
if [ "$FAIL_COUNT" -gt 0 ]; then
    echo ""
    echo -e "${RED}存在 ${FAIL_COUNT} 项检查未通过。${NC}"
    exit 1
else
    echo ""
    echo -e "${GREEN}Glance 镜像服务所有检查项已通过！${NC}"
    exit 0
fi
