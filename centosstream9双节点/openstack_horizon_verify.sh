...
check()   { printf "  %-52s " "$1"; if eval "$2" &>/dev/null; then echo -e "$PASS"; PASS_COUNT=$((PASS_COUNT + 1)); else echo -e "$FAIL"; FAIL_COUNT=$((FAIL_COUNT + 1)); fi }
check_w() { printf "  %-52s " "$1"; if eval "$2" &>/dev/null; then echo -e "$PASS"; PASS_COUNT=$((PASS_COUNT + 1)); else echo -e "$WARN"; WARN_COUNT=$((WARN_COUNT + 1)); fi }
