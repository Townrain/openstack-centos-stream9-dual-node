...
# ==================== 自动检测本机 IP ====================
detect_local_ip() {
    local default_gw; default_gw=$(ip -4 route show default 2>/dev/null | awk '{print $3}' | head -1)
    local default_iface; default_iface=$(ip -4 route show default 2>/dev/null | awk '{print $5}' | head -1)
    local ip; ip=$(ip -4 -o addr show "${default_iface:-ens33}" 2>/dev/null | awk '{print $4}' | cut -d'/' -f1 | head -1)
    echo "$ip"
}
...
remote_compute_mode() {
    ...
    dnf install -y libvirt libvirt-daemon-kvm libvirt-client qemu-kvm qemu-img
...
# ==================== 设置 SSH 免密 ====================
setup_ssh() {
    log_step "配置 SSH 免密登录计算节点"

    [ ! -f /root/.ssh/id_rsa ] && { mkdir -p /root/.ssh; chmod 700 /root/.ssh; ssh-keygen -t rsa -b 2048 -N "" -f /root/.ssh/id_rsa -q; log_info "SSH 密钥已生成"; }

    if ssh -o BatchMode=yes -o ConnectTimeout=10 "${COMPUTE_USER}@${COMPUTE_IP}" "hostname" &>/dev/null 2>&1; then
        log_info "免密登录已就绪"
        return
    fi
