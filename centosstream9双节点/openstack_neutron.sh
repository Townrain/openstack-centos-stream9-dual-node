...
detect_local_ip() {
    local ip; ip=$(ip -4 route show default 2>/dev/null | awk '{print $5; exit}' | xargs -I{} ip -4 -o addr show {} 2>/dev/null | awk '{print $4}' | cut -d'/' -f1 | head -1)
    echo "${ip:-127.0.0.1}"
}

detect_int_ip() {
    # 获取非默认路由网卡的 IP（内部网络）
    local def_iface; def_iface=$(ip -4 route show default 2>/dev/null | awk '{print $5}' | head -1)
...
interactive_main() {
    ...
    echo ""
    echo "============================================"
    echo "  控制节点: ${CTRL_HOSTNAME}  mgmt=${CONTROLLER_IP}  int=${INT_IP} (${INT_IFACE})"
    echo "  计算节点: ${COMPUTE_IP}"
    echo "  metadata 密钥: ${METADATA_SECRET}"
    echo "============================================"
