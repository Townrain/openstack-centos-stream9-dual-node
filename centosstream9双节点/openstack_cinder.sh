...
REMOTE_MODE=0
[ $# -ge 4 ] && REMOTE_MODE=1
backup_file() { [ -f "$1" ] && cp -n "$1" "${1}.bak.$(date '+%Y%m%d%H%M%S')" 2>/dev/null && log_info "已备份 $1"; }

# ==================== 加载环境 ====================
load_env() {
    [ -f /root/openstack_env.conf ] && source /root/openstack_env.conf 2>/dev/null && log_info "已加载 openstack_env.conf"
...
detect_local_ip() {
    ip -4 route show default 2>/dev/null | awk '{print $5; exit}' | xargs -I{} ip -4 -o addr show {} 2>/dev/null | awk '{print $4}' | cut -d'/' -f1 | head -1
}
...
setup_ssh() {
    [ ! -f /root/.ssh/id_rsa ] && { mkdir -p /root/.ssh; chmod 700 /root/.ssh; ssh-keygen -t rsa -b 2048 -N "" -f /root/.ssh/id_rsa -q; }
    ssh -o BatchMode=yes -o ConnectTimeout=5 "${COMPUTE_USER}@${COMPUTE_IP}" "hostname" &>/dev/null && return
    echo ">>> 请输入 ${COMPUTE_USER}@${COMPUTE_IP} 的密码:"
    ssh-copy-id -o StrictHostKeyChecking=no "${COMPUTE_USER}@${COMPUTE_IP}" || { log_error "ssh-copy-id 失败"; exit 1; }
}
...
interactive_main() {
    ...
    if [ "$STORAGE_CHOICE" = "2" ]; then
        ...
        if [ -n "$available" ]; then
            ...
            while IFS= read -r line; do
                ...
                local sz; sz=$(echo "$line" | awk '{print $2}')
                echo "    [$idx] /dev/${dev}  ${sz}"
                idx=$((idx + 1))
            done <<< "$available"

            if [ ${#lines[@]} -gt 1 ]; then
                read -r -p "  请选择磁盘 [1]: " choice; choice="${choice:-1}"
                local selected; selected=$(echo "${lines[$((choice-1))]}" | awk '{print $1}')
                STORAGE_DEV="/dev/${selected}"
            else
                local first; first=$(echo "${lines[0]}" | awk '{print $1}')
                STORAGE_DEV="/dev/${first}"
                log_info "自动选择: /dev/${first}"
            fi
        else
            log_warn "无法检测存储节点磁盘"
            read -r -p "  磁盘设备路径 [/dev/sdb]: " input; STORAGE_DEV="${input:-/dev/sdb}"
        fi
        LOOP_GB=""
    else
    ...
    echo "============================================"
    read -r -p "确认? [y/N]: " cf; [[ ! "$cf" =~ ^[Yy]$ ]] && { log_error "取消"; exit 1; }
