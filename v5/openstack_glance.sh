...
if [ "$NON_INTERACTIVE" -eq 1 ]; then
    ...
    GLANCE_DBPASS="${GLANCE_DBPASS:-${SERVICE_PASS}}"
...
install_packages() {
    ...
    dnf install -y openstack-glance || dnf install -y --allowerasing openstack-glance || { log_error "安装 openstack-glance 失败"; exit 1; }
