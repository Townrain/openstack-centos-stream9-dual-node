...
check "nova.conf [neutron] 已配置"     "grep -A10 '\[neutron\]' /etc/nova/nova.conf | grep -q auth_url"
...
for port in $(ovs-vsctl list-ports br-provider 2>/dev/null); do
    LEAK_IP=$(ip -4 -o addr show "$port" 2>/dev/null | awk '{print $4}' | head -1 || true)
    if [ -n "$LEAK_IP" ]; then
...
if [ -f /root/admin-openrc ]; then
    source /root/admin-openrc 2>/dev/null
    AGENTS=$(openstack network agent list -f value -c 'Binary' -c 'Alive' -c 'Host' 2>/dev/null)
    if [ -n "$AGENTS" ]; then
        while read -r binary alive host; do
            bin_short=$(echo "$binary" | sed 's/neutron-//')
            if [ "$alive" = ":-)" ] || [ "$alive" = "True" ]; then
                echo -e "    ${host}: ${bin_short} ${PASS}"
                PASS_COUNT=$((PASS_COUNT + 1))
            else
                echo -e "    ${host}: ${bin_short} ${WARN}"
                WARN_COUNT=$((WARN_COUNT + 1))
            fi
        done <<< "$AGENTS"

# 检测 OVS 桥端口物理状态（跳过 patch 端口）
echo ""
echo "  OVS 端口状态:"
for port in $(ovs-vsctl list-ports br-provider 2>/dev/null); do
    # Skip OVS virtual patch ports (phy-*, int-*)
    case "$port" in phy-*|int-*) continue ;; esac
    STATE=$(ip -o link show "$port" 2>/dev/null | grep -oP 'state \K\w+' || true)
    if [ "$STATE" = "UP" ]; then
