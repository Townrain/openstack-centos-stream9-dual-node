...
setup_mysql_database() {
    ...
    if ! command -v mysql &>/dev/null; then
        ...
        dnf install -y mariadb || dnf install -y --allowerasing mariadb || { log_error "安装 mariadb 失败"; exit 1; }
    fi

    if ! rpm -q mariadb-server &>/dev/null; then
        log_info "安装 MySQL 服务端..."
        dnf install -y mariadb-server || dnf install -y --allowerasing mariadb-server || { log_error "安装 mariadb-server 失败"; exit 1; }
    fi
...
install_packages() {
    ...
    dnf install -y openstack-keystone httpd python3-mod_wsgi || dnf install -y --allowerasing openstack-keystone httpd python3-mod_wsgi || { log_error "安装 Keystone 软件包失败"; exit 1; }
    log_info "软件包安装完成"
}
...
populate_database() {
    ...
    if [ -s /etc/keystone/fernet-keys/0 ]; then
        log_info "Fernet 密钥已存在且非空，跳过初始化"
    else
